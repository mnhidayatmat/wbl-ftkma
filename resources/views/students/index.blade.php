...
<div class="hidden md:block card-umpsa overflow-hidden">
    @php
        ...
        // Only show Programme column if NOT a WBL Coordinator (they only see their own programme)
        if (!($isWblCoordinator ?? false)) {
            $columns[] = ['label' => 'Programme', 'sortable' => true, 'sortUrl' => $buildSortUrl('programme'), 'sortIcon' => $getSortIcon('programme')];
        }

        $columns[] = ['label' => 'Group', 'sortable' => false];
        $columns[] = ['label' => 'Company', 'sortable' => true, 'sortUrl' => $buildSortUrl('company_id'), 'sortIcon' => $getSortIcon('company_id')];
        $columns[] = ['label' => 'Actions', 'sortable' => false];
    @endphp
    ...
    <x-table :columns="$columns">
        ...
        @empty
            <tr>
                <td colspan="{{ ($isWblCoordinator ?? false) ? 5 : 6 }}" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
