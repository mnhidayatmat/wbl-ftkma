...
@section('title', 'OSH IC Evaluation - ' . $student->name)
...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Back Button -->
        <div class="mb-4">
            <a href="{{ route('academic.osh.ic.index', request()->query()) }}" 
        ...
        @php
            ...
            $completedCount = 0;
            // Calculate completion based on logic in controller/view
            foreach($icAssessments as $assessment) {
                 $hasComponents = $assessment->components->isNotEmpty();
                 $hasRubrics = $assessment->rubrics->isNotEmpty();
                 
                 $isCompleted = false;
                 if ($hasComponents) {
                     $markRecord = $marks->get($assessment->id);
                     if ($markRecord && $markRecord->mark !== null) {
                        $isCompleted = true;
                     }
                 } elseif ($hasRubrics) {
                     $rubricIds = $assessment->rubrics->pluck('id');
                     $filledRubrics = 0;
                     foreach($rubricIds as $rid) {
                         if ($rubricMarks->has($rid)) $filledRubrics++;
                     }
                     if ($filledRubrics === $rubricIds->count() && $filledRubrics > 0) $isCompleted = true;
                 }
                 
                 if ($isCompleted) $completedCount++;
            }
            $progressPercent = $totalAssessments > 0 ? ($completedCount / $totalAssessments) * 100 : 0;
            $maxWeight = $icAssessments->sum('weight_percentage');
        @endphp
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="text-right">
                    ...
                    <div class="text-xs text-gray-500 dark:text-gray-400">out of {{ number_format($maxWeight, 2) }}%</div>
                </div>
            </div>
             <div class="mt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($cloAssessments as $assessment)
                        @php
                            ...
                            // Logic repeated for card display
                            $hasComponents = $assessment->components->isNotEmpty();
                            $hasRubrics = $assessment->rubrics->isNotEmpty();
                            ...
                            if ($isLogbook) {
                                $logbookEvaluations = \App\Models\OshLogbookEvaluation::forStudent($student->id)->get();
                                ...
                                if ($markRecord && $markRecord->mark !== null) {
                                    $isCompleted = true;
                                    $currentMark = $markRecord->mark;
                                    $maxMarkDisplay = $markRecord->max_mark;
                                    if ($markRecord->max_mark > 0) {
                                        $contribution = ($currentMark / $markRecord->max_mark) * $assessment->weight_percentage;
                                    }
                                }
                            } elseif ($hasRubrics) {
                                $rubricIds = $assessment->rubrics->pluck('id');
                                ...
                                $isCompleted = ($filledRubrics === $rubricIds->count() && $filledRubrics > 0);
                                
                                // Calculate rubric contribution
                                foreach($assessment->rubrics as $rubric) {
                                     $m = $rubricMarks->get($rubric->id);
                                     if ($m) $contribution += $m->weighted_contribution;
                                }
                                // For display, maybe show total score if possible, or just contribution
                                $currentMark = $contribution; // Just placeholder for rubrics
                                $maxMarkDisplay = $assessment->weight_percentage; // Just comparison
                            }
                        @endphp
                        
                        @if($isLogbook)
                            {{-- Logbook Card - Links to Logbook Evaluation --}}
                            <a href="{{ route('academic.osh.logbook.show', ['student' => $student->id]) }}"
                               class="group block bg-white dark:bg-gray-800 rounded-xl border-2 {{ $isCompleted ? 'border-green-300 dark:border-green-700' : 'border-gray-200 dark:border-gray-700' }} shadow-sm hover:shadow-lg hover:border-[#0084C5] transition-all duration-300 overflow-hidden flex flex-col h-full">
                            ...
                            <div class="p-5">
                                    ...
                                    <div class="flex items-center gap-2 mt-2">
                                        @if($hasComponents)
                                            <span class="px-2 py-0.5 text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded">
                                                {{ $assessment->components->count() }} Components
                                            </span>
                                        @endif
                                ...
                                <div class="flex items-center justify-between mb-4">
                                    <div>
                                        ...
                                        <span class="font-bold text-gray-900 dark:text-white">{{ $isCompleted ? 'Completed' : 'To Do' }}</span>
                                    </div>
                                    <div class="text-right">
                                ...
                                @can('edit-ic-marks', $student)
                                    <button @click="showModal = true" 
                                        ...
                                        {{ $isCompleted ? 'Edit Evaluation' : 'Evaluate' }}
                                    </button>
                                @else
                            ...
                            <div x-show="showModal" 
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
                                    ...
                                    <div class="flex-1 overflow-y-auto" style="max-height: calc(80vh - 180px);">
                                        <form action="{{ route('academic.osh.ic.store', $student) }}" method="POST" id="form-{{ $assessment->id }}">
                                            @csrf
                                            ...
                                            @if($hasComponents)
                                                ...
                                                <div class="p-4 border-t-2 border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
                                                    ...
                                                    <textarea name="remarks[{{ $assessment->id }}]"
                                                              ...
                                                              placeholder="Add any overall comments or feedback...">{{ $marks->get($assessment->id)?->remarks ?? '' }}</textarea>
                                                </div>
                                            @elseif($hasRubrics)
                                                ...
                                                <div class="p-6 space-y-6">
                                                    @foreach($assessment->rubrics as $rubric)
                                                         @php
                                                            ...
                                                            $rVal = $rMark?->rubric_score;
                                                        @endphp
                                                        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
                                                            <div class="mb-3">
                                                                <div class="flex justify-between items-start">
                                                                    <h5 class="font-bold text-gray-900 dark:text-white">{{ $rubric->question_title }}</h5>
                                                                    <span class="text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-1 rounded">{{ $rubric->weight_percentage }}%</span>
                                                                </div>
                                                                @if($rubric->question_description)
                                                                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ $rubric->question_description }}</p>
                                                                @endif
                                                            </div>
                                                            
                                                            <div class="flex flex-wrap gap-2 mt-4 bg-gray-50 dark:bg-gray-700/30 p-3 rounded-lg">
                                                                @for($i = $rubric->rubric_min; $i <= $rubric->rubric_max; $i++)
                                                                    <label class="cursor-pointer">
                                                                        <input type="radio" 
                                                                               ...
                                                                               {{ $rVal == $i ? 'checked' : '' }}
                                                                               class="peer sr-only">
                                                                        <div class="px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 peer-checked:bg-[#0084C5] peer-checked:text-white peer-checked:border-[#0084C5] transition-all">
                                                                            {{ $i }}
                                                                        </div>
                                                                    </label>
                                                                @endfor
                    ...
                    @endforeach
                                                    ...
                                                    <div class="mt-6">
                                                        ...
                                                        <textarea name="remarks[{{ $assessment->id }}]" rows="3" class="w-full border-gray-300 dark:border-gray-600 rounded-lg focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white p-3">{{ $marks->get($assessment->id)?->remarks }}</textarea>
                                                    </div>
                                                </div>
                                            @else
                                                <div class="p-6">
                                                    <div class="text-center py-10 text-gray-500 bg-white dark:bg-gray-800 rounded-xl border border-dashed border-gray-300">
                                                        <p>No components or rubrics defined for this assessment.</p>
                                                    </div>
                                                </div>
                                            @endif
                                        </form>
        ...
        @empty
             <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-10 text-center">
                ...
                <p class="text-gray-500 mt-2">There are no assessments assigned for IC evaluation.</p>
            </div>
        @endforelse
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($atAssessments as $atAssessment)
                                    @php
                                        ...
                                        $scoreVal = ($atMark && $atMark->marking_type == 'rubric' && $atMark->max_mark <= 5)
                                            ? number_format($atMark->mark, 2) . ' / 5.00'
                                            : number_format($atMark->mark, 2) . ' / ' . number_format($atMark->max_mark, 2);
                                    @endphp
                                    <tr class="bg-white dark:bg-gray-800">
                                        <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                            {{ $atAssessment->assessment_name }}
                                        </td>
                                        <td class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                            {{ $atAssessment->weight_percentage }}%
                                        </td>
                                        <td class="px-6 py-4 text-center font-bold text-[#003A6C] dark:text-[#0084C5]">
                                            {{ $atMark && $atMark->mark !== null ? $scoreVal : '-' }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot class="bg-gray-50 dark:bg-gray-700/50 font-semibold text-gray-900 dark:text-white">
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-center text-[#0084C5]">
                                        {{ number_format($atTotalContribution, 2) }}%
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
