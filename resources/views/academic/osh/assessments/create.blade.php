    ...
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 lg:p-8">
            <form action="{{ route($routePrefix . '.store') }}" method="POST">
                ...
                <div class="mb-6 border border-gray-200 dark:border-gray-700 rounded-lg p-5 bg-white dark:bg-gray-800 shadow-sm">
                    ...
                    <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        @foreach($evaluatorRoles as $key => $label)
                                                <option value="{{ $key }}" {{ old('evaluators.0.role') === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                                    </div>
                                    <div class="w-32">
                                        ...
                                        <input type="number" 
                                               ...
                                               step="0.01"
                                               placeholder="0.00"
                                               value="{{ old('evaluators.0.total_score') }}"
                                               onchange="validateEvaluatorTotal()"
                                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm">
                                    </div>
                ...
                <!-- Question Configuration (for Oral/Rubric/Report types) -->
...
const oldEvaluators = @json(old('evaluators', []));
const cloCodes = @json($cloCodes);

let componentIndex = Array.isArray(oldComponents) ? Object.keys(oldComponents).length : 0;
...
function updateTotalCloWeight() {
    ...
    // Update evaluator validation as total CLO weight changed
    if (typeof validateEvaluatorTotal === 'function') {
        validateEvaluatorTotal();
    }
}
...
function addComponent(data = null) {
    ...
    const assessmentType = assessmentTypeSelect ? assessmentTypeSelect.value : '';
    const isRubricType = ['Oral', 'Rubric'].includes(assessmentType);
    ...
    const cloOptionsHtml = availableCloCodes.map(clo => 
        `<option value="${clo}" ${clo === (data?.clo_code || defaultClo) ? 'selected' : ''}>${clo}</option>`
                ...
                <div>
                    ...
                    <input type="number" 
                           name="components[${currentCount}][min_score]" 
                ...
                <div>
                    ...
                    <input type="number" 
                           name="components[${currentCount}][max_score]" 
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="components[${currentCount}][example_answer]" 
                              rows="2"
                              placeholder="Optional guidance or example answer for this component..."
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm">${data?.example_answer ?? ''}</textarea>
                </div>
            </div>
...
function removeComponent(button) {
    if (confirm('Are you sure you want to remove this component?')) {
        ...
        components.forEach((component, index) => {
            ...
            if (titleSpan) {
                // Check if it's a Component or Question (based on existing text or just set generic index)
                // Use existing structure to guess type, or simply use "Item" if unsure? 
                // Better: check if it has "Question Title" label to decide "Question X" vs "Component X"
                const hasQuestionTitle = component.querySelector('input[name*="question_title"]');
                titleSpan.textContent = hasQuestionTitle ? `Question ${index + 1}` : `Component ${index + 1}`;
            }
...
function validateComponentWeights() {
    // Select both 'components' and 'rubrics' weight inputs
    const weightInputs = document.querySelectorAll('#componentsContainer input[name*="[weight_percentage]"]');
    let total = 0;
    weightInputs.forEach(input => {
        const value = parseFloat(input.value) || 0;
        total += value;
...
function addEvaluator(data = null) {
    ...
    for (const [key, label] of Object.entries(evaluatorRoles)) {
        const isSelected = data && data.role === key ? 'selected' : '';
        roleOptions += `<option value="${key}" ${isSelected}>${label}</option>`;
    }
...
// Repopulate dynamic evaluators (skip first one as it's handled by HTML)
if (Array.isArray(oldEvaluators) && oldEvaluators.length > 1) {
    // Start from index 1 since index 0 is the default row
    for (let i = 1; i < oldEvaluators.length; i++) {
        addEvaluator(oldEvaluators[i]);
    }
}
</script>
@endsection
