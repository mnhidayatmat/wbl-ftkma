@php
    $routePrefix = 'academic.osh.assessments';
@endphp
@extends('layouts.app')
...
<div class="py-6 overflow-x-hidden">
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
            <form action="{{ route($routePrefix . '.update', $assessment) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div id="rubricSection" class="mb-6 border-t border-gray-200 dark:border-gray-700 pt-6" style="display: {{ $assessment->assessment_type !== 'Logbook' ? 'block' : 'none' }};">
                    ...
                    <div id="componentsContainer" class="space-y-4">
                        @php
                            $mergedComponents = collect([]);
                            if(isset($components) && $components) $mergedComponents = $mergedComponents->concat($components);
                            
                            // Migrating existing rubrics to components view for seamless transition
                            if(isset($rubrics) && $rubrics && $rubrics->count() > 0) {
                                $mappedRubrics = $rubrics->map(function($r) {
                                    return (object)[
                                        'id' => null, // force new creation as component
                                        'component_name' => $r->question_title,
                                        'criteria_keywords' => $r->question_description,
                                        'clo_code' => $r->clo_code,
                                        'weight_percentage' => $r->weight_percentage,
                                        'min_score' => $r->rubric_min,
                                        'max_score' => $r->rubric_max,
                                        'example_answer' => $r->example_answer ?? '',
                                        'order' => $r->order ?? 0
                                    ];
                                });
                                $mergedComponents = $mergedComponents->concat($mappedRubrics);
                            }
                            // Sort by order
                            $mergedComponents = $mergedComponents->sortBy('order')->values();
                        @endphp
                    ...
                    <div class="mb-6">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-5 border border-gray-200 dark:border-gray-700 mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                        Number of Months/Periods <span class="text-red-500">*</span>
                                    </label>
                                    <input type="number" 
                                           ...
                                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm">
                                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Total number of evaluation periods (e.g., 6 for 6 months)</p>
                                </div>
...
function addCloRow() {
    ...
    // Add event listener to the new CLO select - update component CLO options when CLO changes
    ...
    validateEvaluatorTotal();
    validateCloWeightsAgainstTotal();
    updateComponentCloCodes(); // Update component CLO dropdowns with new CLO options
}
    ...
    if (container) {
        ...
        inputs.forEach(input => {
         ...
         allInputs.forEach(input => {
            // Only sum components, rubrics are gone
...
function addComponent() {
    ...
    componentDiv.setAttribute('data-order', currentIndex);
    
    componentDiv.innerHTML = `
        <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
                <span class="text-sm font-semibold text-[#003A6C] dark:text-[#0084C5]">Component ${currentIndex + 1}</span>
            </div>
        ...
        <input type="hidden" name="components[${currentIndex}][order]" value="${currentIndex}" class="component-order">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                    Component Name <span class="text-red-500">*</span>
                </label>
                <input type="text" name="components[${currentIndex}][component_name]" required placeholder="e.g. Problem Statement, Project Objectives" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                    Criteria Keywords
                </label>
                <textarea name="components[${currentIndex}][criteria_keywords]" rows="2" placeholder="e.g. Clarity and Conciseness, Relevance, Specificity, Value-added Quality, and Originality" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm"></textarea>
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                    CLO Code <span class="text-red-500">*</span>
                </label>
                <select name="components[${currentIndex}][clo_code]" 
                        required
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm">
                    <option value="">Select CLO</option>
                    ${getAllSelectedCloCodes().map((clo, i) => `<option value="${clo}" ${i === 0 ? 'selected' : ''}>${clo}</option>`).join('')}
                </select>
            ...
            <div>
                ...
                <input type="number" name="components[${currentIndex}][weight_percentage]" required step="0.01" min="0" max="${totalCloWeight}" onchange="calculateTotalWeight()" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                    Min Score
                </label>
                <input type="number" name="components[${currentIndex}][min_score]" step="0.01" min="0" placeholder="e.g. 0.00" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                    Max Score
                </label>
                <input type="number" name="components[${currentIndex}][max_score]" step="0.01" min="0" placeholder="e.g. 10.00" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                    Example Answer / Guidance (Optional)
                </label>
                <textarea name="components[${currentIndex}][example_answer]" rows="2" placeholder="Optional guidance or example answer for this component..." class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white text-sm"></textarea>
            </div>
        </div>
    `;
...
function updateComponentOrder() {
    ...
    components.forEach((component, index) => {
        ...
        if (orderInput) {
            orderInput.value = index;
            // Update the name attribute to reflect new index for components only
            const oldName = orderInput.name;
            if (oldName.includes('components')) {
                 orderInput.name = `components[${index}][order]`;
            }
        }
        ...
        component.querySelectorAll('input, select, textarea').forEach(input => {
            if (input.name && input.name.includes('components[')) {
                // Simplified replacement as we know we are only dealing with components now
                // Regex to find components[X] and replace with components[index]
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize view based on assessment type
    const currentType = document.getElementById('assessment_type').value;
    if (['Oral', 'Rubric'].includes(currentType)) {
        toggleRubricType('question');
    } else {
