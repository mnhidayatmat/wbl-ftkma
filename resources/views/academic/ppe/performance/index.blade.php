...
@section('title', 'Student Performance Overview')
...
<div class="py-6 overflow-x-hidden">
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex items-center justify-between flex-wrap gap-4">
            <div>
                <h1 class="text-2xl font-bold text-[#003A6C] dark:text-[#0084C5]">Student Performance Overview</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-2">Overall performance based on Lecturer ({{ $lecturerTotalWeight }}%) and Industry Coach ({{ $icTotalWeight }}%) evaluations</p>
            </div>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" 
                     ...
                     style="display: none;">
                    <div class="py-1">
                        <a href="{{ route('academic.ppe.performance.export.excel', request()->query()) }}" 
                        ...
                        <a href="{{ route('academic.ppe.performance.export.pdf', request()->query()) }}" 
                           class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5 border-l-4 border-blue-500">
                ...
                <div class="text-2xl font-black text-[#003A6C] dark:text-[#0084C5]">{{ number_format($lecturerTotalWeight, 2) }}%</div>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5 border-l-4 border-green-500">
                ...
                <div class="text-2xl font-black text-green-600 dark:text-green-400">{{ number_format($icTotalWeight, 2) }}%</div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 sm:p-6 mb-6">
            <form method="GET" action="{{ route('academic.ppe.performance.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
                <!-- Search -->
                <div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-[#003A6C]">
                            <tr>
                                ...
                                <th class="px-3 sm:px-4 lg:px-6 py-2 sm:py-3 text-left text-xs font-medium text-white uppercase tracking-wider hidden md:table-cell">IC</th>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                ...
                                <td class="px-3 sm:px-4 lg:px-6 py-3 sm:py-4">
                                    <div class="text-xs sm:text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                        {{ number_format($student->lecturer_score, 1) }}%<span class="text-gray-500">/{{ $lecturerTotalWeight }}%</span>
                                    </div>
                                    <!-- Progress Bar -->
                                ...
                                <td class="px-3 sm:px-4 lg:px-6 py-3 sm:py-4 hidden md:table-cell">
                                    <div class="text-xs sm:text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                        {{ number_format($student->ic_score, 1) }}%<span class="text-gray-500">/{{ $icTotalWeight }}%</span>
                                    </div>
                                    <!-- Progress Bar -->
                                    <div class="mt-1 max-w-[80px] sm:max-w-[100px] bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                                        <div class="bg-[#00AEEF] h-1.5 rounded-full" 
                                             style="width: {{ min($student->ic_progress, 100) }}%"></div>
                                    </div>
                                </td>
                                <td class="px-3 sm:px-4 lg:px-6 py-3 sm:py-4">
