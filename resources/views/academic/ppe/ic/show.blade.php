...
@section('title', 'PPE IC Evaluation - ' . $student->name)
...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Back Button -->
        <div class="mb-4">
            <a href="{{ route('academic.ppe.ic.index', request()->query()) }}" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($cloAssessments as $assessment)
                        @php
                            ...
                            if ($isLogbook) {
                                $logbookEvaluations = \App\Models\PpeLogbookEvaluation::forStudent($student->id)->get();
                        ...
                        @if($isLogbook)
                            ...
                            <a href="{{ route('academic.ppe.logbook.show', ['student' => $student->id]) }}"
                            ...
                            <div class="p-5">
                                ...
                                @can('edit-ic-marks', $student)
                                    <button @click="showModal = true"
                                            class="w-full py-3 px-4 bg-gradient-to-r from-[#0084C5] to-[#003A6C] hover:from-[#003A6C] hover:to-[#002850] text-white font-medium rounded-lg transition-all duration-300 flex items-center justify-center gap-2 shadow-md hover:shadow-lg">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div x-show="showModal" 
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
                                    ...
                                    <div class="flex-1 overflow-y-auto" style="max-height: calc(80vh - 180px);">
                                        <form action="{{ route('academic.ppe.ic.store', $student) }}" method="POST" id="form-{{ $assessment->id }}">
                                            @csrf
                                                            ...
                                                            <div class="flex flex-wrap gap-2">
                                                                @for($i=$rubric->rubric_min; $i<=$rubric->rubric_max; $i++)
                                                                     <label class="cursor-pointer">
                                                                        <input type="radio" name="rubric_scores[{{ $rubric->id }}]" value="{{ $i }}" {{ $rScore == $i ? 'checked' : '' }} class="peer sr-only">
                    ...
                    @endforeach
                                                    ...
                                                    <div class="mt-6">
                                                        ...
                                                        <textarea name="remarks[{{ $assessment->id }}]" rows="3" class="w-full border-gray-300 dark:border-gray-600 rounded-lg focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white p-3">{{ $mark?->remarks }}</textarea>
                                                    </div>
                                                </div>
                                            @else
                                                <div class="p-6">
                                                    <div class="text-center py-10 text-gray-500">
                                                        No components or rubrics defined for this assessment.
                                                    </div>
                                                </div>
                                            @endif
                                        </form>
        ...
        @empty
             <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-10 text-center">
                ...
                <p class="text-gray-500 mt-2">There are no assessments assigned for Industry Coach evaluation yet.</p>
            </div>
        @endforelse
