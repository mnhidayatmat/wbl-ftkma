                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($assessments as $assessmentName => $assessmentGroup)
                                @php
                                    ...
                                    // Aggregated Evaluators display
                                    $evaluatorDisplay = $assessmentGroup->map(function($a) {
                                        $evaluators = $a->evaluators ?? collect([]);
                                        return $evaluators->count() > 0 
                                            ? $evaluators->pluck('evaluator_role')->map(fn($r) => ucfirst(str_replace('_', ' ', $r)))->unique()
                                            : collect([ucfirst(str_replace('_', ' ', $a->evaluator_role))]);
                                    })->flatten()->unique()->join(', ');
                                @endphp
                                ...
                                <tr class="bg-gray-50 dark:bg-gray-800/50 border-l-4 border-[#0084C5]">
                                    <td class="px-4 sm:px-6 lg:px-8 py-4" colspan="7">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('academic.ppe.assessments.rubric-report.show', $firstAssessment) }}"
                                                   class="p-2 {{ $firstAssessment->rubricReport ? 'text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20' : 'text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700' }} rounded-lg transition-colors"
                                                   title="{{ $firstAssessment->rubricReport ? 'View Rubric Report' : 'Add Rubric Report' }}">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                    </svg>
                                                </a>
                                                <a href="{{ route($routePrefix . '.edit', $firstAssessment) }}"
                                                ...
                                                <form action="{{ route($routePrefix . '.destroy', $firstAssessment) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this assessment and all its CLOs? This action cannot be undone.')">
                                            @csrf
                                            @method('DELETE')
                                ...
                                @foreach($allClos as $clo)
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                        ...
                                        <td class="px-4 sm:px-6 lg:px-8 py-3.5">
                                            <div class="flex items-center gap-1.5">
                                                <form action="{{ route($routePrefix . '.toggle-active', $clo->assessment_id) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" 
                                                ...
                                                <a href="{{ route($routePrefix . '.edit', $clo->assessment_id) }}" 
