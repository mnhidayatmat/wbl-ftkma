@php
    $routePrefix = 'academic.fyp.assessments';
@endphp
@extends('layouts.app')
...
<div class="py-6 overflow-x-hidden">
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
            <form action="{{ route('academic.fyp.assessments.update', $assessment) }}" method="POST">
                @csrf
                @method('PUT')
...
// Generate logbook components based on number of months
function generateLogbookComponents() {
    const numberOfMonths = parseInt(document.getElementById('logbookNumberOfMonths')?.value || 6);
    const availableCloCodes = getAllSelectedCloCodes();
    ...
    const weightPerMonth = totalWeight / numberOfMonths;
    const defaultClo = getDefaultCloCode();
    
    // Generate components
    for (let i = 0; i < numberOfMonths; i++) {
        addLogbookComponent({
            duration_label: `Month ${i + 1}`,
            clo_code: defaultClo,
            weight_percentage: weightPerMonth.toFixed(2),
            criteria_keywords: '',
            order: i
        });
    }
...
function addLogbookComponent(data = null) {
    ...
    const monthNumber = currentCount + 1;
    const componentDiv = document.createElement('div');
    ...
    componentDiv.innerHTML = `
        <div class="flex items-center justify-between mb-4">
            <span class="text-sm font-semibold text-[#003A6C] dark:text-[#0084C5]">Period ${monthNumber}</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" 
                       ...
                       value="${data?.duration_label || 'Month ' + monthNumber}"
                       required
