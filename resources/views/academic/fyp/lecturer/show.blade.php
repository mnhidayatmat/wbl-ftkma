...
@section('title', 'FYP AT Evaluation - ' . $student->name)
...
<div class="py-6">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Back Button -->
        <div class="mb-4">
            <a href="{{ route('academic.fyp.lecturer.index', request()->query()) }}" 
               class="inline-flex items-center text-[#00A86B] hover:text-[#003A6C] transition-colors">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-16 h-16 bg-gradient-to-br from-[#00A86B] to-[#003A6C] rounded-full flex items-center justify-center text-white text-2xl font-bold">
                        {{ strtoupper(substr($student->name, 0, 1)) }}
                    </div>
                    <div>
                        <h1 class="text-xl font-bold text-[#003A6C] dark:text-[#00A86B]">{{ $student->name }}</h1>
        ...
        @php
            ...
            $canEdit = auth()->user()->isAdmin() || (auth()->user()->isAt() && $student->at_id == auth()->id());
        @endphp
        
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">AT Evaluation Progress</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $completedAssessments }} of {{ $totalAssessments }} assessments completed</p>
                </div>
                <div class="flex items-center gap-4">
                    <div class="text-right">
                        <div class="text-3xl font-bold text-[#00A86B]">{{ number_format($totalContribution, 2) }}%</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">out of {{ number_format($totalAtWeight, 2) }}%</div>
                    </div>
                </div>
            </div>
            <div class="mt-4">
            ...
            @foreach($groupedAssessments as $phase => $assessmentGroups)
                ...
                <div class="mb-8">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-[#003A6C] dark:text-[#00A86B]">{{ $phaseInfo['label'] }}</h3>
                        ...
                        @foreach($assessmentGroups as $baseName => $assessmentsInGroup)
                            ...
                            @if($isLogbook)
                                ...
                                <a href="{{ route('academic.fyp.logbook.show', ['student' => $student->id]) }}"
                                   class="group block bg-white dark:bg-gray-800 rounded-xl border-2 {{ $isCompleted ? 'border-green-300 dark:border-green-700' : 'border-gray-200 dark:border-gray-700' }} shadow-sm hover:shadow-lg hover:border-[#00A86B] transition-all duration-300 overflow-hidden">
                                    ...
                                    <div class="p-5 relative">
                                        <div class="mb-4">
                                            <h4 class="font-semibold text-[#003A6C] dark:text-[#00A86B] pr-16 leading-tight group-hover:text-[#00A86B]">
                                                {{ $baseName }}
                                            </h4>
                                        ...
                                        <div class="flex items-center justify-between mb-4">
                                            ...
                                            <div class="text-right">
                                                ...
                                                <span class="text-lg font-bold text-[#00A86B]">{{ number_format($groupContribution, 2) }}%</span>
                                                <span class="text-xs text-gray-400 block">of {{ number_format($groupTotalWeight, 2) }}%</span>
                                            </div>
                                        </div>

                                        <div class="w-full py-2.5 px-4 bg-[#00A86B] group-hover:bg-[#003A6C] text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2">
                            ...
                            @elseif($assessmentsInGroup->count() > 1)
                                ...
                                <div class="group relative bg-white dark:bg-gray-800 rounded-xl border-2 {{ $isCompleted ? 'border-green-300 dark:border-green-700' : ($isPartial ? 'border-amber-300 dark:border-amber-700' : 'border-gray-200 dark:border-gray-700') }} shadow-sm hover:shadow-lg hover:border-[#00A86B] transition-all duration-300 overflow-hidden"
                                    ...
                                    <div class="p-5">
                                        <div class="mb-4">
                                            <h4 class="font-semibold text-[#003A6C] dark:text-[#00A86B] pr-16 leading-tight">
                                                {{ $baseName }}
                                            </h4>
                                            <div class="flex items-center gap-2 mt-2">
                                        ...
                                        <button @click="showModal = true" 
                                                class="w-full py-2.5 px-4 bg-[#00A86B] hover:bg-[#003A6C] text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div x-show="showModal" 
                                        ...
                                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
                                            ...
                                            <!-- Modal Header -->
                                            <div class="flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-[#003A6C] to-[#00A86B] flex-shrink-0">
                                            ...
                                            <div class="flex-1 overflow-y-auto" style="max-height: calc(80vh - 180px);">
                                                <form action="{{ route('academic.fyp.lecturer.store', $student) }}" method="POST" id="groupForm-{{ $firstAssessmentId }}">
                                                    @csrf
                                                        ...
                                                        <table class="w-full">
                                                            ...
                                                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                                                ...
                                                                @foreach($components as $index => $component)
                                                                    ...
                                                                    @endphp
                                                                    <tr class="hover:bg-green-50/50 dark:hover:bg-gray-700/50 transition-colors {{ !$assessment ? 'opacity-60' : '' }} component-row" 
                                                                            ...
                                                                            <div class="flex items-start gap-2">
                                                                                ...
                                                                                <div class="flex flex-col gap-1 mt-0.5">
                                                                                    <button type="button" 
                                                                                            ...
                                                                                            class="text-[#00A86B] hover:text-[#003A6C] disabled:text-gray-300 disabled:cursor-not-allowed transition-colors"
                                                                                            {{ $index === 0 ? 'disabled' : '' }}
                                                                                            title="Move Up">
                                                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                        ...
                                                                        @if($assessment)
                                                                            ...
                                                                            <td class="py-4 px-3 text-center align-middle rating-cell-wrapper transition-all duration-200">
                                                                                <label class="rubric-radio-label inline-flex items-center justify-center cursor-pointer w-full py-3 rounded-md transition-all duration-200
                                                                                    ...
                                                                                    <span class="sr-only">{{ $template['rating_labels'][$score - 1] ?? ['Aware', 'Limited', 'Fair', 'Good', 'Excellent'][$score - 1] }}</span>
                                                                                </label>
                                                                            </td>
                                                                            @endfor
                                                                        @else
                                            ...
                                            <!-- Modal Footer -->
                                            <div class="flex gap-3 p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex-shrink-0">
                                                <button type="button" 
                            ...
                            @else
                                ...
                                <div class="group relative bg-white dark:bg-gray-800 rounded-xl border-2 {{ $isSingleCompleted ? 'border-green-300 dark:border-green-700' : 'border-gray-200 dark:border-gray-700' }} shadow-sm hover:shadow-lg hover:border-[#00A86B] transition-all duration-300 overflow-hidden"
                                     x-data="{ showModal: false }">
                                    ...
                                    <div class="p-5">
                                        ...
                                        <div class="flex items-center justify-between mb-4">
                                            ...
                                            <div class="text-right">
                                                <span class="text-xs text-gray-500 dark:text-gray-400 block">Contribution</span>
                                                <span class="text-lg font-bold text-[#00A86B]">{{ number_format($contribution, 2) }}%</span>
                                    ...
                                    <!-- Modal for Single Assessment with Rubric Grid -->
                                    <div x-show="showModal" 
                                         x-transition:enter="transition ease-out duration-300"
                                         x-transition:enter-start="opacity-0"
                                         x-transition:enter-end="opacity-100"
                                        ...
                                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
                                            ...
                                            <!-- Modal Body -->
                                            <div class="flex-1 overflow-y-auto" style="max-height: calc(80vh - 180px);">
                                                <form action="{{ route('academic.fyp.lecturer.store', $student) }}" method="POST" id="singleForm-{{ $assessment->id }}">
                                                    @csrf
                                                            ...
                                                            <tbody class="bg-white dark:bg-gray-800">
                                                                @foreach($components as $index => $component)
                                                                    ...
                                                                    <tr class="rubric-row border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors" 
                                                                        ...
                                                                        <!-- Left Column: Component Info (Fixed Width) -->
                                                                        <td class="py-4 px-6 align-top">
                                                                            <div class="space-y-2">
                                                                                ...
                                                                                <button type="button" 
                                                                                        ...
                                                                                        class="text-xs text-[#00A86B] hover:text-[#003A6C] dark:text-[#00A86B] dark:hover:text-[#0084C5] mt-1 flex items-center gap-1 transition-colors font-medium">
                                                                                ...
                                                                                <div x-show="showRemarks" 
                                                                                    ...
                                                                                    <textarea name="component_remarks[{{ $hasComponents && $component->id ? $component->id : 'temp_' . $index }}]" 
                                                                                              ...
                                                                                              class="w-full px-3 py-2 text-xs border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-[#00A86B] focus:border-[#00A86B] resize-none">{{ $componentMark?->remarks }}</textarea>
                                                                        ...
                                                                        <!-- Rating Columns: Radio Buttons (Equal Width) -->
                                                                        @for($score = 1; $score <= 5; $score++)
                                                                        <td class="py-4 px-3 text-center align-middle rating-cell-wrapper transition-all duration-200">
                                                                            <label class="rubric-radio-label inline-flex items-center justify-center cursor-pointer w-full py-3 rounded-md transition-all duration-200
                                                    ...
                                                    <div class="p-4 border-t-2 border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
                                                        ...
                                                        <textarea name="remarks[{{ $assessment->id }}]"
                                                                  rows="3"
                                                                  class="w-full px-4 py-3 border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-[#00A86B] focus:border-[#00A86B] transition-all resize-none"
        ...
        @else
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-12 text-center">
                ...
                <p class="text-gray-500 dark:text-gray-400">
                    No assessments configured for FYP AT evaluation yet.<br>
...
function updateRubricSelection(radioInput) {
    ...
    allRadios.forEach(radio => {
        radio.classList.remove('border-[#0084C5]', 'dark:border-[#00A86B]', 'bg-[#0084C5]', 'dark:bg-[#00A86B]');
    ...
    if (radioInput.checked) {
        ...
        radioInput.classList.add('border-[#0084C5]', 'dark:border-[#00A86B]', 'bg-[#0084C5]', 'dark:bg-[#00A86B]');
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('form[id^="singleForm-"], form[id^="groupForm-"]').forEach(form => {
        const rows = form.querySelectorAll('tr.rubric-row');
        let allComplete = true;
