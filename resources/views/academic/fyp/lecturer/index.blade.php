...
@section('title', 'FYP – AT Evaluation')
...
<div class="py-4 sm:py-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-4 sm:mb-6">
            <h1 class="text-xl sm:text-2xl font-bold text-[#003A6C] dark:text-[#0084C5]">FYP – AT Evaluation</h1>
            <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-2">Manage and enter assessment marks for assigned students</p>
        </div>

        <!-- Filters -->
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
            <form method="GET" action="{{ route('academic.fyp.lecturer.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <!-- Search -->
                <div>
                ...
                <div class="flex items-end space-x-2 sm:col-span-2 lg:col-span-1">
                    ...
                    <a href="{{ route('academic.fyp.lecturer.index') }}" 
                       class="px-4 py-2 sm:py-2.5 bg-gray-300 hover:bg-gray-400 text-gray-700 rounded-lg transition-colors min-h-[44px] flex items-center">
                        Clear
                    </a>
                </div>
            </form>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-[#003A6C]">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Total Contribution</th>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @elseif($student->evaluation_status == 'in_progress')
                                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-300">
                                            In Progress ({{ $student->completed_assessments }}/{{ $student->total_assessments }})
                                        </span>
                                    @else
                                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-300">
                                            Completed
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">
                                    <span class="font-medium">{{ number_format($student->total_contribution, 2) }}%</span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    @if(auth()->user()->isAdmin() || (auth()->user()->isAt() && $student->at_id == auth()->id()))
                                        <a href="{{ route('academic.fyp.lecturer.show', $student) }}" 
                                    ...
                                    <div class="flex flex-col items-center">
                                        ...
                                        @if(auth()->user()->isAt() && !auth()->user()->isAdmin())
                                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                                Please contact an administrator to assign students to you as Academic Tutor (AT).
                                            </p>
                                        @endif
                                    </div>
                                </td>
