...
<div class="py-6 overflow-x-hidden">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <div class="text-base font-semibold text-gray-900 dark:text-white">{{ $student->company->company_name ?? 'N/A' }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">Supervisor Score</div>
                    <div class="text-2xl font-bold text-[#0084C5]">{{ number_format($supervisorTotal, 2) }}%</div>
                </div>
                <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">IC Score</div>
                    <div class="text-2xl font-bold text-[#00AEEF]">{{ number_format($icTotal, 2) }}%</div>
                </div>
                <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">Final Score</div>
                    <div class="text-2xl font-bold text-[#003A6C] dark:text-[#0084C5]">{{ number_format($finalScore, 2) }}%</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
            ...
            <form method="POST" action="{{ route('academic.li.moderation.store', $student) }}" x-data="{ adjustmentType: '{{ $moderation->adjustment_type ?? 'percentage' }}' }">
                @csrf
                ...
                <div x-show="adjustmentType === 'percentage'" class="mb-6">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number"
                               ...
                               min="-100"
                               max="100"
                               x-bind:required="adjustmentType === 'percentage'"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white">
                ...
                <div x-show="adjustmentType === 'manual_override'" class="mb-6 space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                            Adjusted Supervisor Score
                        </label>
                        <input type="number"
                               name="adjusted_supervisor_score"
                               value="{{ $moderation->adjusted_supervisor_score ?? $supervisorTotal }}"
                               step="0.01"
                               min="0"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white">
                    </div>
