...
@section('title', 'Industrial Training – Supervisor Evaluation - ' . $student->name)
...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Back Button -->
        <div class="mb-4">
            <a href="{{ route('academic.li.lecturer.index', request()->query()) }}" 
        ...
        <!-- Progress Summary -->
        @php
            $totalAssessmentsCount = $assessments->count();
            $completedCount = 0;
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                     <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Supervisor Evaluation Progress</h2>
                            ...
                            <div x-show="showModal" 
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
                                    ...
                                    <div class="flex-1 overflow-y-auto" style="max-height: calc(80vh - 180px);">
                                        <form action="{{ route('academic.li.lecturer.store', $student) }}" method="POST" id="form-{{ $assessment->id }}">
                                            @csrf
                                                ...
                                                <div class="p-4 border-t-2 border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
                                                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                                                        <span class="flex items-center gap-2">
                                                            ...
                                                            Summative Feedback / Supervisor Remarks (Optional)
                                                        </span>
                                                    </label>
                                                    <textarea name="remarks"
                                                              ...
                                                              placeholder="Enter comments on student's overall performance during the internship...">{{ $mark?->remarks ?? '' }}</textarea>
                                                </div>
                                            @elseif($assessment->rubrics->isNotEmpty())
                                                ...
                                                <div class="p-6 space-y-6">
                                                            ...
                                                            <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                                                                @for($i=$rubric->rubric_min; $i<=$rubric->rubric_max; $i++)
                                                                    <label class="cursor-pointer group flex flex-col gap-2">
                                                                        ...
                                                                        <div class="py-4 text-center border-2 border-gray-100 dark:border-gray-700 rounded-2xl font-bold text-gray-400 peer-checked:border-[#0084C5] peer-checked:bg-blue-50 dark:peer-checked:bg-blue-900/20 peer-checked:text-[#0084C5] transition-all">
                                                    ...
                                                    <!-- Overall Remarks -->
                                                    <div class="mt-6">
                                                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Summative Feedback / Supervisor Remarks</label>
                                                    ...
                                                    <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-200 dark:border-gray-700 p-8 shadow-sm max-w-lg mx-auto">
                                                        <div class="grid grid-cols-2 gap-6">
                                                            <div>
                                                                ...
                                                                <input type="number" step="0.01" name="mark" value="{{ $mark?->mark }}" class="w-full text-2xl font-black border-gray-200 dark:border-gray-700 rounded-2xl focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white p-4" placeholder="0.00">
                                                            </div>
                                                            <div>
        ...
        @empty
             <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-md p-16 text-center border-2 border-dashed border-gray-100 dark:border-gray-800">
                ...
                <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">No Assessments Assigned</h3>
                <p class="text-gray-500 text-sm mt-2 max-w-sm mx-auto">There are currently no active assessments for the Supervisor LI role for this student.</p>
            </div>
        @endforelse

        <!-- Read-Only Section for Industry Coach -->
        @if($icAssessments->isNotEmpty())
            <div class="mt-12 pt-12 border-t-2 border-dashed border-gray-100 dark:border-gray-700">
                <div class="flex items-center gap-3 mb-8">
                    <div class="p-3 bg-green-50 dark:bg-green-900/30 rounded-2xl border border-green-100 dark:border-green-800">
                        <svg class="w-6 h-6 text-[#10b981]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                        </svg>
                    </div>
                    <div>
                        <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Industry Coach Evaluation</h3>
                        <p class="text-[10px] text-green-600 dark:text-green-400 font-black uppercase tracking-[0.2em]">Marks synchronized from external coach</p>
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                                @foreach($icAssessments as $icAssessment)
                                    @php
                                        ...
                                        $mVal = $icMark?->mark;
                                        $maxV = $icMark?->max_mark ?? 100;
                                        $cVal = ($mVal !== null && $maxV > 0) ? ($mVal / $maxV) * $icAssessment->weight_percentage : 0;
                                    @endphp
                                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors">
                                        ...
                                        <td class="px-8 py-5 text-right font-black text-[#10b981]">
                            ...
                            <tfoot class="bg-gray-50/50 dark:bg-gray-700/50">
                                <tr>
                                    <td class="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-500">Total IC Contribution</td>
                                    <td colspan="2" class="px-8 py-6 text-right text-2xl font-black text-[#10b981]">
