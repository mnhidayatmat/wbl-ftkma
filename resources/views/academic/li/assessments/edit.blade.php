@php
    $routePrefix = 'academic.li.assessments';
@endphp
@extends('layouts.app')
...
<div class="py-6 overflow-x-hidden">
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
            <form action="{{ route('academic.li.assessments.update', $assessment) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div id="rubricSection" class="mb-6 border-t border-gray-200 dark:border-gray-700 pt-6" style="display: {{ $assessment->assessment_type !== 'Logbook' ? 'block' : 'none' }};">
                    ...
                    <!-- Questions Container (Hidden - Not used for FYP) -->
    ...
    if (container.querySelectorAll('.clo-row').length > 1) {
        ...
        rows.forEach((row, index) => {
            ...
            if (orderInput) {
                ...
                orderInput.name = orderInput.name.replace(/\[order\]/, `[${index}][order]`);
            }
...
// Function to disable reordering for all rubric questions
function disableRubricReordering() {
    document.querySelectorAll('.rubric-question').forEach(rubric => {
        // Remove any component-item class that might have been added
        rubric.classList.remove('component-item');
        rubric.classList.remove('cursor-move');
        
        // Set draggable to false (use both attribute and property)
        rubric.setAttribute('draggable', 'false');
        rubric.draggable = false;
        rubric.setAttribute('data-no-reorder', 'true');
        
        // Prevent all drag events using capture phase (runs BEFORE other handlers)
        ...
        // Add event listeners at capture phase to intercept before component handlers
        ['dragstart', 'dragover', 'drop', 'dragenter', 'dragleave', 'dragend'].forEach(eventType => {
            rubric.addEventListener(eventType, preventDrag, true); // true = capture phase
        });
        
        // Also set inline handlers as backup (these have highest priority)
        rubric.ondragstart = function() { return false; };
        rubric.ondragover = function() { return false; };
        rubric.ondrop = function() { return false; };
        rubric.ondragenter = function() { return false; };
        rubric.ondragleave = function() { return false; };
        
        // Apply CSS styles to prevent visual drag indicators
        rubric.style.cursor = 'default';
        rubric.style.userSelect = 'none';
        rubric.style.webkitUserDrag = 'none';
        rubric.style.pointerEvents = 'auto';
        
        // Ensure all child elements also cannot be dragged
        rubric.querySelectorAll('*').forEach(child => {
            child.style.webkitUserDrag = 'none';
            child.ondragstart = function() { return false; };
        });
    });
}
...
function toggleRubricType(type) {
    ...
    if (type === 'question') {
        ...
        // Disable reordering for rubric questions when shown
        setTimeout(() => {
            disableRubricReordering();
...
function addRubricQuestion() {
    ...
    // Ensure the newly added rubric question cannot be dragged
    questionDiv.setAttribute('draggable', 'false');
    questionDiv.setAttribute('data-no-reorder', 'true');
    questionDiv.style.cursor = 'default';
    questionDiv.style.userSelect = 'none';
    questionDiv.style.webkitUserDrag = 'none';
    // Ensure it doesn't have component-item class
    questionDiv.classList.remove('component-item');
    questionDiv.classList.remove('cursor-move');
    
    // Call the disable function to ensure full protection
    setTimeout(() => {
        disableRubricReordering();
    }, 50);
    validateRubricWeights();
...
function validateWeights() {
    ...
    // Default to 'component' if no radio button is found (FYP case), otherwise usage existing logic
...
function updateWeightValidation(total, type) {
    ...
    if (statusDiv && validationDiv) {
        ...
        if (totalCloWeight === 0) {
            ...
            validationDiv.className = 'mt-4 p-3 rounded-lg bg-gray-50 dark:bg-gray-900/20 border border-gray-200 dark:border-gray-800';
...
function toggleRubricSection() {
    ...
    if (assessmentType === 'Logbook') {
        ...
        // Ensure rubric questions cannot be dragged when section is shown
        setTimeout(() => {
            disableRubricReordering();
        }, 100);
...
function addComponent() {
    ...
    // Only setup drag-and-drop if it's NOT a rubric question
    if (!componentDiv.classList.contains('rubric-question') && !componentDiv.hasAttribute('data-no-reorder')) {
...
function setupComponentDragAndDrop(element) {
    ...
    if (element.classList.contains('rubric-question') || 
        element.hasAttribute('data-no-reorder') ||
        element.closest('#rubricsContainer') ||
        element.closest('.rubric-question')) {
        return; // Skip rubric questions - reordering is disabled
    }
    
    element.addEventListener('dragstart', function(e) {
        // Double-check it's not a rubric question (multiple checks)
        if (this.classList.contains('rubric-question') || 
            this.hasAttribute('data-no-reorder') ||
            this.closest('#rubricsContainer')) {
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation();
            return false;
    ...
    element.addEventListener('dragover', function(e) {
        // Prevent drag-over on rubric questions (multiple checks)
        if (this.classList.contains('rubric-question') || 
            this.hasAttribute('data-no-reorder') ||
            this.closest('#rubricsContainer')) {
            e.preventDefault();
    ...
    element.addEventListener('dragenter', function(e) {
        // Don't highlight rubric questions (multiple checks)
        if (this.classList.contains('rubric-question') || 
            this.hasAttribute('data-no-reorder') ||
            this.closest('#rubricsContainer')) {
            e.preventDefault();
    ...
    element.addEventListener('dragleave', function(e) {
        // Don't process dragleave for rubric questions (multiple checks)
        if (this.classList.contains('rubric-question') || 
            this.hasAttribute('data-no-reorder') ||
            this.closest('#rubricsContainer')) {
            e.preventDefault();
    ...
    element.addEventListener('drop', function(e) {
        ...
        if (draggedElement && draggedElement !== this) {
            // Multiple checks to ensure neither element is a rubric question
            const isDraggedRubric = draggedElement.classList.contains('rubric-question') || 
                                   draggedElement.hasAttribute('data-no-reorder') ||
                                   draggedElement.closest('#rubricsContainer');
            const isTargetRubric = this.classList.contains('rubric-question') || 
                                  this.hasAttribute('data-no-reorder') ||
                                  this.closest('#rubricsContainer');
            
            if (!isDraggedRubric && !isTargetRubric) {
                // Ensure both are component-item, not rubric-question
                if (this.classList.contains('component-item')) {
                    const allComponents = Array.from(document.querySelectorAll('.component-item:not(.rubric-question):not([data-no-reorder])'));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.component-item').forEach(component => {
        // Double-check: skip if it's a rubric question
        if (component.classList.contains('rubric-question') || 
            component.hasAttribute('data-no-reorder') ||
            component.closest('#rubricsContainer')) {
            return; // Skip rubric questions
        }
        setupComponentDragAndDrop(component);
    });
    
    // CRITICAL: Ensure NO drag handlers are applied to rubric questions
    // Run immediately and also after delays to catch any late initialization
    disableRubricReordering();
    setTimeout(() => {
        disableRubricReordering();
    }, 200);
    ...
    // Also run periodically to catch any dynamically added or modified rubric questions
    // This is a safety net in case any code tries to enable dragging
    setInterval(() => {
        const rubricQuestions = document.querySelectorAll('.rubric-question');
        if (rubricQuestions.length > 0) {
            rubricQuestions.forEach(rubric => {
                // Quick check and fix - ensure draggable is always false
                if (rubric.getAttribute('draggable') !== 'false') {
                    rubric.setAttribute('draggable', 'false');
                    rubric.draggable = false;
                }
                if (!rubric.hasAttribute('data-no-reorder')) {
                    rubric.setAttribute('data-no-reorder', 'true');
                }
                if (rubric.classList.contains('component-item')) {
                    rubric.classList.remove('component-item');
                }
                if (rubric.classList.contains('cursor-move')) {
                    rubric.classList.remove('cursor-move');
                }
                // Ensure CSS styles are applied
                if (rubric.style.cursor !== 'default') {
                    rubric.style.cursor = 'default';
                }
                if (rubric.style.webkitUserDrag !== 'none') {
                    rubric.style.webkitUserDrag = 'none';
                }
            });
        }
    }, 2000); // Check every 2 seconds (less frequent to avoid performance issues)
    
    // Explicitly disable drag-and-drop for all rubric questions
    disableRubricReordering();
    
    // Use MutationObserver to catch any dynamically added rubric questions
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1) { // Element node
                    if (node.classList && node.classList.contains('rubric-question')) {
                        disableRubricReordering();
                    }
                    // Also check children
                    const rubricQuestions = node.querySelectorAll && node.querySelectorAll('.rubric-question');
    ...
    // Observe the rubricsContainer for any changes
    const rubricsContainer = document.getElementById('rubricsContainer');
    if (rubricsContainer) {
        observer.observe(rubricsContainer, {
            childList: true,
            subtree: true
        });
    }
    
    // Also observe the rubricSection
    const rubricSection = document.getElementById('rubricSection');
    if (rubricSection) {
        observer.observe(rubricSection, {
            childList: true,
            subtree: true
        });
    }
