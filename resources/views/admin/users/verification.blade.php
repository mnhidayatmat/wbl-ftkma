...
@section('title', 'User Email Verification Management')

@section('content')
<div class="container mx-auto px-4 py-8 max-w-7xl">
    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">User Email Verification</h1>
        <p class="text-gray-600 dark:text-gray-400">Manage email verification for all users</p>
    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">{{ $verifiedUsers }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600 dark:text-red-400 mt-1">{{ $unverifiedUsers }}</p>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex flex-col md:flex-row gap-4 flex-1">
                <form method="GET" action="{{ route('admin.users.verification.index') }}" class="flex-1">
                    <div class="flex gap-2">
                        <input 
                            ...
                            class="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-umpsa-teal focus:border-umpsa-teal dark:bg-gray-700 dark:text-gray-100"
                        >
                        <button 
                            type="submit"
                            class="px-4 py-2 bg-umpsa-teal text-white rounded-lg hover:bg-umpsa-royal-blue transition-colors"
                        >
                            Search
                        </button>
                        @if(request('search'))
                            <a 
                                href="{{ route('admin.users.verification.index') }}"
                ...
                <!-- Status Filter -->
                <form method="GET" action="{{ route('admin.users.verification.index') }}">
                    <input type="hidden" name="search" value="{{ request('search') }}">
                    <select 
                        name="status" 
                        onchange="this.form.submit()"
                        class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-umpsa-teal focus:border-umpsa-teal dark:bg-gray-700 dark:text-gray-100"
                    >
            ...
            @if($unverifiedUsers > 0)
                <form method="POST" action="{{ route('admin.users.verification.send-all') }}" onsubmit="return confirm('Send verification emails to all unverified users?');">
                    @csrf
                    <button 
                        type="submit"
                        class="px-4 py-2 bg-umpsa-teal text-white rounded-lg hover:bg-umpsa-royal-blue transition-colors whitespace-nowrap"
                    >
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <div class="flex items-center gap-2">
                                    @if(!$user->hasVerifiedEmail())
                                        <form method="POST" action="{{ route('admin.users.verification.send', $user) }}" class="inline">
                                            @csrf
                                            <button 
                                                type="submit"
                                                class="px-3 py-1 bg-umpsa-teal text-white rounded-lg hover:bg-umpsa-royal-blue transition-colors text-xs"
                                            >
...
<script>
    function openEditRolesModal(userId, currentRoleIds) {
        ...
        // Set form action
        form.action = `/admin/users/verification/${userId}/roles`;
