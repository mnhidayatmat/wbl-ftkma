            ...
            <form action="{{ route('admin.documents.sal.update') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label for="sal_release_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            SAL Release Date <span class="text-red-500">*</span>
                        </label>
                        <button type="button" onclick="document.getElementById('sal_release_date').value=''" class="text-xs text-red-500 hover:text-red-700 font-medium flex items-center gap-1">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label for="sal_reference_number" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            SAL Reference Number <span class="text-red-500">*</span>
                        </label>
                        <button type="button" onclick="document.getElementById('sal_reference_number').value=''" class="text-xs text-red-500 hover:text-red-700 font-medium flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                            Reset
                        </button>
                    </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label for="director_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Director Name
                        </label>
                        <button type="button" onclick="document.getElementById('director_name').value=''" class="text-xs text-red-500 hover:text-red-700 font-medium flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                            Reset
                        </button>
                    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">SAL Auto-Release</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-0.5">
                        When enabled, SAL will automatically be generated and released when a student is assigned to a group.
                    </p>
                    @if($template->settings['sal_auto_release_enabled'] ?? false)
                        <div class="flex items-center gap-2 mt-2">
                            <span class="inline-flex items-center gap-1.5 text-xs bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-2.5 py-1 rounded-full font-medium">
                                <span class="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                                Auto-Release Active
                            </span>
                            @if($template->settings['sal_auto_release_enabled_at'] ?? null)
                                <span class="text-xs text-gray-500 dark:text-gray-400">
                                    Enabled {{ \Carbon\Carbon::parse($template->settings['sal_auto_release_enabled_at'])->diffForHumans() }}
                                </span>
                            @endif
                        </div>
                    @else
            ...
            <form action="{{ route('admin.documents.sal.toggle-auto-release') }}" method="POST">
                @csrf
                @if($template->settings['sal_auto_release_enabled'] ?? false)
                    <button type="submit"
                            onclick="return confirm('Disable SAL auto-release? Students will no longer receive SAL automatically when they are assigned to a group.')"
                ...
                @else
                    <button type="submit"
                            onclick="return confirm('Enable SAL auto-release? SAL will automatically be generated when students are assigned to a group.')"
                            class="px-5 py-2.5 bg-[#003A6C] hover:bg-[#002855] text-white font-semibold rounded-lg transition-colors flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        Enable SAL Auto-Release
                    </button>
                @endif
            </form>
        </div>
