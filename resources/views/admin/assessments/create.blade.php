...
<div class="py-6">
    <div class="max-w-4xl mx-auto px-10">
        <div class="mb-6">
            <a href="{{ route('admin.assessments.index', ['course' => $courseCode]) }}" 
            ...
            <h1 class="text-2xl font-bold heading-umpsa">Create New Assessment</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-2">Define a new assessment for {{ $courseCodes[$courseCode] }}</p>
        </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="course_code" 
                        ...
                        @foreach($courseCodes as $code => $name)
                            <option value="{{ $code }}" {{ $courseCode === $code ? 'selected' : '' }}>
                                {{ $code }} - {{ $name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    @if($courseCode === 'FYP')
                        ...
                        <div class="mb-4">
                            ...
                            <div id="closContainer" class="space-y-3">
                                <!-- CLO entries will be added here dynamically -->
                            </div>
                        ...
                        <div class="mb-6">
                            ...
                            <select name="clo_code" 
                                ...
                                @foreach($cloCodes as $clo)
                                    <option value="{{ $clo }}" {{ old('clo_code') === $clo ? 'selected' : '' }}>
                                        {{ $clo }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="mb-6">
                    ...
                    <select name="evaluator_role" 
                        ...
                        @foreach($evaluatorRoles as $key => $label)
                            <option value="{{ $key }}" {{ old('evaluator_role') === $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
        ...
        cloSection.innerHTML = `
            <div class="mb-6">
                ...
                <select name="clo_code" id="clo_code" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#0084C5] focus:border-[#0084C5] dark:bg-gray-700 dark:text-white">
                    ${clos.map(clo => `<option value="${clo}">${clo}</option>`).join('')}
                </select>
            </div>
        `;
    }
...
function addRubricQuestion() {
    const weightInput = document.querySelector('input[name="weight_percentage"]');
    if (weightInput) {
        assessmentWeight = parseFloat(weightInput.value) || 0;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize CLO section for FYP
    @if($courseCode === 'FYP')
        if (document.getElementById('closContainer')) {
            addCloEntry(); // Add first CLO entry
        }
    @endif
