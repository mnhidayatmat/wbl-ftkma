@php
    ...
    // Module Coordinators
    $isFypCoordinator = $user->isFypCoordinator();
    $isIpCoordinator = $user->isIpCoordinator();
...
<nav class="mt-4 space-y-1" :class="isSidebarCollapsed ? 'px-2' : 'px-3'"
    ...
    <!-- Group #1: Dashboard (Hidden for Students and Module Coordinators - they have their own dashboards) -->
    @if(!$isStudent && !$isModuleCoordinator)
    ...
    <!-- Module Coordinator Dashboard (for all module coordinators except FYP which has its own section) -->
    ...
    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        <!-- Recruitment Pool -->
        <a href="{{ route('recruitment.pool.index') }}"
           class="flex items-center gap-1 rounded-lg transition-all duration-300 ease-in-out min-h-[44px] {{ request()->routeIs('recruitment.*') ? 'bg-[#E6F4EF] dark:bg-gray-700/50 text-[#003A6C] dark:text-white border-l-[3px] border-[#00A86B] font-medium' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}"
           :class="isSidebarCollapsed ? 'justify-center px-0' : 'px-2'"
           :title="isSidebarCollapsed ? 'Recruitment Pool' : ''">
            <div class="w-9 h-9 flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            ...
            <span x-show="sidebarTextVisible" x-transition class="text-sm font-medium">Recruitment Pool</span>
        </a>
        @endif
    ...
    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        <div class="mb-1">
            ...
            <div x-show="ipMenuOpen && sidebarTextVisible" 
                 ...
                 x-transition:leave-end="opacity-0 -translate-y-2"
                 class="ml-6 mt-1 space-y-1">
                @if($isAdmin || $isIpCoordinator)
                <a href="{{ route('academic.ip.assign-students.index') }}"
                ...
                @endif
                @if($isAdmin || $isCoordinator || $isLecturer || $isIpCoordinator)
                ...
                @endif
                @if($isLecturer || $isAdmin || $isIpCoordinator)
                ...
                @endif
                @if($isIc || $isAdmin || $isIpCoordinator)
    ...
    <!-- Group #3: INDUSTRY COACH (Hidden for all Module Coordinators) -->
    @if($isIc && !$isFypCoordinator && !$isIpCoordinator && !$isOshCoordinator && !$isPpeCoordinator && !$isLiCoordinator)
