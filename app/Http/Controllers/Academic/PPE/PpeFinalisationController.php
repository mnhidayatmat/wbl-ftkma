...
class PpeFinalisationController extends Controller
{
    /**
    ...
    public function index(Request $request): View
    {
        ...
        if ($request->filled('status')) {
            if ($request->status === 'finalised') {
                $finalisedStudentIds = PpeResultFinalisation::where('is_finalised', true)
        ...
        $finalisations = PpeResultFinalisation::whereIn('student_id', $students->pluck('id'))
        ...
        $finalisedCount = PpeResultFinalisation::where('is_finalised', true)
            ->whereNotNull('student_id')
            ->distinct('student_id')
            ->count();
        $notFinalisedCount = $totalStudents - $finalisedCount;

        return view('academic.ppe.finalisation.index', compact(
            'students',
            'finalisations',
            'groups',
            'totalStudents',
    ...
    public function finaliseStudent(Request $request, Student $student)
    {
        ...
        $existing = PpeResultFinalisation::where('student_id', $student->id)
            ->where('is_finalised', true)
            ->first();
        ...
        $finalisation = PpeResultFinalisation::updateOrCreate(
        ...
        return redirect()->route('academic.ppe.finalisation.index')
    ...
    public function finaliseCourse(Request $request)
    {
        ...
        PpeAuditLog::log(
            'result_finalised',
            'finalisation',
            "Results finalised for entire PPE course ({$finalisedCount} students)",
            [
                'finalisation_scope' => 'course',
                'students_finalised' => $finalisedCount,
            ]
