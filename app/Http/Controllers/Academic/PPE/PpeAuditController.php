...
class PpeAuditController extends Controller
{
    /**
    ...
    public function index(Request $request): View
    {
        ...
        // Build query
        $query = PpeAuditLog::with(['user', 'student', 'assessment']);
        ...
        $actionTypes = PpeAuditLog::distinct()->pluck('action_type')->sort()->values();

        // Get unique actions for filter
        $actions = PpeAuditLog::distinct()->pluck('action')->sort()->values();

        // Get users for filter
        $users = \App\Models\User::whereIn('id', PpeAuditLog::distinct()->pluck('user_id'))
            ->orderBy('name')
            ->get();
        ...
        return view('academic.ppe.audit-log.index', [
            'auditLogs' => $auditLogs,
    ...
    public function export(Request $request)
    {
        ...
        // Generate CSV
        $filename = 'ppe_audit_log_'.now()->format('Y-m-d_His').'.csv';
        $headers = [
