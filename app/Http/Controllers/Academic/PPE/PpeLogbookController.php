...
class PpeLogbookController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $logbookAssessment = \App\Models\Assessment::forCourse('PPE')
        ...
        $allEvaluations = PpeLogbookEvaluation::whereIn('student_id', $students->pluck('id'))
            ->get()
            ->groupBy('student_id');
        ...
        $studentsWithStatus = $students->map(function($student) use ($allEvaluations, $totalPeriods) {
            ...
            $student->completed_months = $completedPeriods;
        ...
        return view('academic.ppe.logbook.index', compact(
            'studentsWithStatus',
            'groups'
        ));
    }
    ...
    public function show(Student $student): View
    {
        ...
        if (!$logbookAssessment) {
            abort(404, 'Logbook assessment not found. Please create a Logbook assessment for Professional Practice & Ethics.');
        }
        ...
        $evaluations = PpeLogbookEvaluation::forStudent($student->id)
            ->get()
            ->keyBy('month');
        ...
        foreach ($logbookComponents as $index => $component) {
            $periodIndex = $component->order;
            $evaluation = $evaluations->get($periodIndex);
            
            $periods->push([
                'period_index' => $periodIndex,
                'component_id' => $component->id,
                'label' => $component->duration_label,
        ...
        return view('academic.ppe.logbook.show', compact(
            'student',
            'periods',
            'totalScore',
            'completedPeriods',
    ...
    public function store(Request $request, Student $student): RedirectResponse
    {
        ...
        foreach ($validPeriodIndices as $periodIndex) {
            ...
            if ($score !== null) {
                PpeLogbookEvaluation::updateOrCreate(
                    [
        ...
        return redirect()->route('academic.ppe.logbook.show', $student)
