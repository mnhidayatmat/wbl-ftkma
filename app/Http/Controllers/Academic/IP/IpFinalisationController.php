...
use App\Models\IP\IpResultFinalisation;
use App\Models\Student;
use App\Models\WblGroup;
use Illuminate\Http\Request;
use Illuminate\View\View;

class IpFinalisationController extends Controller
{
    /**
    ...
    public function index(Request $request): View
    {
        ...
        if ($request->filled('status')) {
            if ($request->status === 'finalised') {
                $finalisedStudentIds = IpResultFinalisation::where('is_finalised', true)
        ...
        $finalisations = IpResultFinalisation::whereIn('student_id', $students->pluck('id'))
        ...
        $finalisedCount = IpResultFinalisation::where('is_finalised', true)
            ->whereNotNull('student_id')
            ->distinct('student_id')
            ->count();
        $notFinalisedCount = $totalStudents - $finalisedCount;

        return view('academic.ip.finalisation.index', compact(
            'students',
            'finalisations',
            'groups',
            'totalStudents',
    ...
    public function finaliseStudent(Request $request, Student $student)
    {
        ...
        $existing = IpResultFinalisation::where('student_id', $student->id)
            ->where('is_finalised', true)
            ->first();
        ...
        $finalisation = IpResultFinalisation::updateOrCreate(
        ...
        return redirect()->route('academic.ip.finalisation.index')
