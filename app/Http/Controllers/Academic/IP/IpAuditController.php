...
class IpAuditController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        // Build query
        $query = IpAuditLog::with(['user', 'student', 'assessment']);
        ...
        $actionTypes = IpAuditLog::distinct()->pluck('action_type')->sort()->values();

        // Get unique actions for filter
        $actions = IpAuditLog::distinct()->pluck('action')->sort()->values();

        // Get users for filter
        $users = \App\Models\User::whereIn('id', IpAuditLog::distinct()->pluck('user_id'))
            ->orderBy('name')
            ->get();
        ...
        return view('academic.ip.audit.index', [
            'auditLogs' => $auditLogs,
