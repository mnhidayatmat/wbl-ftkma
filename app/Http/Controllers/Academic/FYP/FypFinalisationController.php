...
use App\Models\FYP\FypResultFinalisation;
use App\Models\Student;
use App\Models\WblGroup;
use Illuminate\Http\Request;
use Illuminate\View\View;

class FypFinalisationController extends Controller
{
    /**
    ...
    public function index(Request $request): View
    {
        ...
        if ($request->filled('status')) {
            if ($request->status === 'finalised') {
                $finalisedStudentIds = FypResultFinalisation::where('is_finalised', true)
        ...
        $finalisations = FypResultFinalisation::whereIn('student_id', $students->pluck('id'))
        ...
        $finalisedCount = FypResultFinalisation::where('is_finalised', true)
            ->whereNotNull('student_id')
            ->distinct('student_id')
            ->count();
        $notFinalisedCount = $totalStudents - $finalisedCount;

        return view('academic.fyp.finalisation.index', compact(
            'students',
            'finalisations',
            'groups',
            'totalStudents',
    ...
    public function finaliseStudent(Request $request, Student $student)
    {
        ...
        $existing = FypResultFinalisation::where('student_id', $student->id)
            ->where('is_finalised', true)
            ->first();
        ...
        $finalisation = FypResultFinalisation::updateOrCreate(
        ...
        return redirect()->route('academic.fyp.finalisation.index')
    ...
    public function finaliseGroup(Request $request)
    {
        ...
        foreach ($students as $student) {
            ...
            if (! $existing) {
                FypResultFinalisation::updateOrCreate(
                    ['student_id' => $student->id],
                    [
                        'group_id' => $group->id,
    ...
    public function finaliseCourse(Request $request)
    {
        ...
        FypAuditLog::log(
            'result_finalised',
            'finalisation',
            "Results finalised for entire FYP course ({$finalisedCount} students)",
            [
                'finalisation_scope' => 'course',
                'students_finalised' => $finalisedCount,
            ]
