...
class OshStudentAssignmentController extends Controller
{
    /**
    ...
    public function index(Request $request): View
    {
        ...
        // Get the currently assigned OSH lecturer (check if all students have same lecturer)
        $currentLecturer = $this->getCurrentOshLecturer();
        ...
        return view('academic.osh.assign-students.index', compact(
            'students',
            'lecturers',
            'groups',
            'stats',
    ...
    /**
     * Get the current OSH lecturer assigned to students.
     * Returns the lecturer if all students have the same one, null otherwise.
     */
    private function getCurrentOshLecturer(): ?User
    {
    ...
    /**
     * Assign a single lecturer to ALL students (OSH module specific).
     */
    ...
    public function export(Request $request)
    {
        ...
        $headers = [
            ...
            'Content-Disposition' => 'attachment; filename="osh-student-assignments.csv"',
        ];

        $callback = function () use ($students) {
            $file = fopen('php://output', 'w');
            fputcsv($file, ['No', 'Student Name', 'Matric No', 'Programme', 'Group', 'Lecturer', 'Industry Coach', 'Company (IC)', 'Company Address']);
