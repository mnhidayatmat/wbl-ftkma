...
class OshAuditController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        // Build query
        $query = OshAuditLog::with(['user', 'student', 'assessment']);
        ...
        $actionTypes = OshAuditLog::distinct()->pluck('action_type')->sort()->values();

        // Get unique actions for filter
        $actions = OshAuditLog::distinct()->pluck('action')->sort()->values();

        // Get users for filter
        $users = \App\Models\User::whereIn('id', OshAuditLog::distinct()->pluck('user_id'))
            ->orderBy('name')
            ->get();
        ...
        return view('academic.osh.audit.index', [
            'auditLogs' => $auditLogs,
    ...
    public function export(Request $request)
    {
        ...
        // Generate CSV
        $filename = 'osh_audit_log_' . now()->format('Y-m-d_His') . '.csv';
        $headers = [
