...
class OshIcEvaluationController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        // Get active assessments for OSH course with IC evaluator role
        $icAssessments = Assessment::forCourse('OSH')
            ->forEvaluator('ic')
            ->active()
        ...
        // For rubrics: count of rubrics
        // For components: count of assessments (since we grade by assessment usually, or check assessment completion)
        // Let's count "Assessments" as the unit for status to be consistent with Show page progress bar
        ...
        $studentsWithStatus = $students->map(function($student) use ($allRubricMarks, $allMarks, $icAssessments, $totalAssessments) {
            ...
            // Use placeholders if not fully implementing granular counts for index
        ...
        return view('academic.osh.ic.index', compact(
            'studentsWithStatus',
            'groups',
            'totalIcWeight'
        ));
    ...
    public function show(Student $student): View
    {
        ...
        // Get active rubric-based and component-based assessments for OSH course with IC evaluator role
        $icAssessments = Assessment::forCourse('OSH')
            ->forEvaluator('ic')
            ->active()
        ...
        $atAssessments = Assessment::forCourse('OSH')
        ...
        return view('academic.osh.ic.show', compact(
            'student',
            'icAssessments',
            'assessmentsByClo',
            'rubricMarks',
    ...
    public function store(Request $request, Student $student): RedirectResponse
    {
        ...
        // Get active assessments for OSH course with IC evaluator role
        // This variable was missing in previous refactor causing Undefined variable $icAssessments
        $icAssessments = Assessment::forCourse('OSH')
            ->forEvaluator('ic')
            ->active()
            // Support both Rubric and Component based
        ...
        if ($request->has('component_marks')) {
            ...
            foreach ($processedAssessmentIds as $assessmentId) {
                 if (!$validAssessmentIds->contains($assessmentId)) continue;
        ...
        return redirect()->route('academic.osh.ic.show', $student)
