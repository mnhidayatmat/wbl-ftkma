...
class OshModerationController extends Controller
{
    /**
    ...
    public function index(Request $request): View
    {
        ...
        $moderationRecords = OshModerationRecord::whereIn('student_id', $students->pluck('id'))
        ...
        $totalLecturerWeight = $lecturerAssessments->sum('weight_percentage');
        $totalIcWeight = $icAssessments->sum('weight_percentage');

        return view('academic.osh.moderation.index', compact(
            'studentsWithScores',
            'groups',
            'totalLecturerWeight',
            'totalIcWeight'
    ...
    public function show(Student $student): View
    {
        ...
        $moderation = OshModerationRecord::where('student_id', $student->id)->first();

        // Calculate total weights for display
        $totalLecturerWeight = $lecturerAssessments->sum('weight_percentage');
        $totalIcWeight = $icAssessments->sum('weight_percentage');

        return view('academic.osh.moderation.show', compact(
            'student',
            'lecturerTotal',
            'icTotal',
            'finalScore',
    ...
    public function store(Request $request, Student $student)
    {
        ...
        $validated = $request->validate([
            ...
            'adjusted_lecturer_score' => ['nullable', 'numeric', 'min:0'],
        ...
        $moderation = OshModerationRecord::updateOrCreate(
        ...
        return redirect()->route('academic.osh.moderation.index')
