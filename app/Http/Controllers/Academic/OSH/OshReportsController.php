...
class OshReportsController extends Controller
{
    /**
    ...
    public function index(): View
    {
        ...
        return view('academic.osh.reports.index', compact(
            'totalStudents',
            'totalGroups',
            'totalCompanies'
        ));
    ...
    public function exportCohort(Request $request)
    {
        ...
        // Log export
        Log::info('OSH Cohort Report Export', [
            'admin_id' => auth()->id(),
        ...
        if ($format === 'pdf') {
            return $this->exportPdf($studentsWithPerformance, 'OSH Full Cohort Results');
        }

        $fileName = 'OSH_Cohort_Results_'.now()->format('Y-m-d_His').'.xlsx';
    ...
    public function exportGroup(Request $request, WblGroup $group)
    {
        ...
        // Log export
        Log::info('OSH Group Report Export', [
            'admin_id' => auth()->id(),
            'admin_name' => auth()->user()->name,
            'format' => $format,
            'group_id' => $group->id,
        ...
        if ($format === 'pdf') {
            return $this->exportPdf($studentsWithPerformance, "OSH Results - {$group->name}");
        }

        $fileName = 'OSH_Group_'.str_replace(' ', '_', $group->name).'_'.now()->format('Y-m-d_His').'.xlsx';
    ...
    public function exportCompany(Request $request, Company $company)
    {
        ...
        // Log export
        Log::info('OSH Company Report Export', [
            'admin_id' => auth()->id(),
            'admin_name' => auth()->user()->name,
            'format' => $format,
        ...
        if ($format === 'pdf') {
            return $this->exportPdf($studentsWithPerformance, "OSH Results - {$company->company_name}");
        }

        $fileName = 'OSH_Company_'.str_replace(' ', '_', $company->company_name).'_'.now()->format('Y-m-d_His').'.xlsx';
